...
export function AccountCard({ account, onEdit, onDelete, disabled = false }: AccountCardProps) {
  ...
  const handleCardClick = () => {
    ...
    router.push('/adventure-packs'); // Navigate to adventure packs for this account
  };
    ...
    >
      ...
      >
        <CardHeader className="p-4">
          ...
          <CardDescription className="text-primary-foreground/80 font-medium">Manage this account's packs</CardDescription>
        </CardHeader>
      </div>
      <CardFooter className="flex justify-end gap-2 p-4 pt-2 border-t border-white/20 mt-auto">
        <Button variant="outline" size="sm" onClick={() => onEdit(account)} aria-label={`Edit ${account.name}`} disabled={disabled} className="bg-white/10 text-white border-white/30 hover:bg-white/20">
          <Pencil className="mr-2 h-4 w-4" /> Edit
        </Button>
        <Button variant="destructive" size="sm" onClick={() => onDelete(account.id)} aria-label={`Delete ${account.name}`} disabled={disabled}>
